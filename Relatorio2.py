...
def processar_e_enviar():
    ...
    for dia, dados in sorted(dados_por_dia.items()):
        ...
        for conta_num in sorted(contas.keys()):
            ...
            relatorio = f"""Conta de anuncio Meta Ads {conta_num}
...
""" + "\n".join([f"📣 {nome} - Ocorrências: {count}" for nome, count in top_ad_sets]) + """

✨ Melhores Criativos (Ad Name):
""" + "\n".join([f"🎨 {nome} - Ocorrências: {count}" for nome, count in top_ad_names])
...
1. Gere *INSIGHTS* com base nos melhores públicos e criativos apenas 3 topicos e resumido.
2. Gere *PRÓXIMOS PASSOS* que o time pode tomar apenas 3 topicos e resumido.
